%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
%  Merge multiple postprocessing files from the different CPUs            %
%                                                                         %
...
%
%
matlab_cpus = UserSettings.gen_options.matlab_cpus;
%
...
fprintf(fid,'    RVEs_data = pickle.load(readfile_postproc)\n');
...
fprintf(fid,'file_postproc_path_CPU%s = postproc_dir+''/''+''RVEs_postprocessing_variables_CPU%s.p''\n',num2str(iCPU),num2str(iCPU));
...
fprintf(fid,'    readfile_postproc_CPU%s = open(file_postproc_path_CPU%s, ''r'')\n',num2str(iCPU),num2str(iCPU));
fprintf(fid,'    RVEs_data_CPU%s = pickle.load(readfile_postproc_CPU%s)\n',num2str(iCPU),num2str(iCPU));
...
fprintf(fid,'    RVEs_data = dict_merge(RVEs_data, RVEs_data_CPU%s)\n',num2str(iCPU));
fprintf(fid,'elif (postproc_exists != 1) and (postproc_exists_CPU%s == 1):\n',num2str(iCPU));
fprintf(fid,'    RVEs_data = RVEs_data_CPU%s\n',num2str(iCPU));
...
disp(' ');
unix(strcat(UserSettings.analyses.abaqus_path,' cae noGUI=',postproc_dir,'/script_merge_postproc_files.py'));
disp(' ');
disp('Finished updating the merged RVEs_postprocessing_variables.p file');
